export function renderArchitectureView(t, archImages, onOpenLightbox) {
  ...
  container.innerHTML = `
    <div class="max-w-7xl mx-auto">
      ...
      <div id="arch-gallery-justified" class="justified-gallery w-full"></div>
    </div>
  `;

  const galleryEl = container.querySelector('#arch-gallery-justified');
    ...
    images.forEach((img, idx) => {
      ...
      if (currentTotalWidth >= containerWidth) {
        ...
        row.forEach(item => {
          ...
          div.innerHTML = `
            <img src="${item.thumb || item.src}" alt="Architecture by Faik Nagiyev" loading="lazy" decoding="async" />
            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none flex items-center justify-center">
              <span class="p-3 bg-black/60 text-white backdrop-blur-sm">
  ...
  function update() {
    ...
    renderJustified(archImages, targetH, 12);
  }
